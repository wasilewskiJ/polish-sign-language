...
def run_stratified_kfold_cv(X, y, model, model_name, n_splits=5, seed=42, results_dir=None):
    """
...
def make_tf_dataset(files, labels, img_size=(128, 128), batch_size=32, shuffle=False, seed=42):
...
def run_cnn_cv(raw_dir, n_splits=5, seed=42, img_size=(128, 128), results_dir=None):
    """
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(all_files, y_encoded), 1):
        ...
        # Create datasets
        train_ds = make_tf_dataset(files_train, y_train, img_size=img_size, batch_size=32, shuffle=True, seed=seed)
        val_ds = make_tf_dataset(files_val, y_val, img_size=img_size, batch_size=32, shuffle=False)
...
def main():
    ...
    print("LOADING DATA")
    print("="*70)
    X, y = load_raw_data_and_extract_features(raw_dir)
    ...
    print("="*70)
    print(f"\n{'Model':<20} {'Accuracy':<25} {'F1-macro':<25}")
    print("-"*70)
